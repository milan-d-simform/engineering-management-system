  ...
  <div>
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">{{ usersStore.total }} members in your team</p>
      </div>
      <AppButton @click="showCreateModal = true">+ New Member</AppButton>
    </div>
    ...
    >
      <template #toolbar>
        <div class="flex flex-wrap gap-2 items-center">
          ...
          />
          <button
            v-if="search || filterRole || filterProject"
            type="button"
    ...
    <Teleport to="body">
      <div v-if="showCreateModal || editUser" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 my-4">
          <h3 class="text-lg font-semibold mb-4">{{ editUser ? 'Edit Member' : 'Add Member' }}</h3>
      ...
      <div v-if="resetPwdUser" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
          ...
          <AppInput v-model="newPassword" label="New Password" type="password" required :error="pwdError" />
...
// Short debounce batches simultaneous changes (e.g. clearFilters resetting all at once).
let filterTimer: ReturnType<typeof setTimeout>;
watch([search, filterRole, filterProject], () => {
...
function openEdit(user: User) { editUser.value = user; }
...
async function handleUserSubmit(payload: CreateUserPayload | UpdateUserPayload) {
  ...
  try {
    if (editUser.value) {
      ...
      successMsg.value = 'Member updated successfully';
    } else {
...
async function handleResetPassword() {
  pwdError.value = '';
  if (newPassword.value.length < 8) { pwdError.value = 'Min 8 characters'; return; }
